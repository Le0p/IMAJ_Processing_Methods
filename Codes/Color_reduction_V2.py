...
def quantized_colors_with_Kmeans(pixels, pixel_type, image, num_colors_max, pixels_HSV = None):
    ...
    for num_colors in tqdm(range(1, num_colors_max)):
        ...
        if pixel_type == 'HSV':
            mse_HSV = mean_squared_error(pixels_HSV, kmeans.cluster_centers_[labels])*num_colors
...
for n in range(30):
    image_path = f'Test_images_{n}.png'
    ...
    opt_num_color.append(find_optimal_color_number(HSV_error_tab[1]))
